...
test('inventário baseline é determinístico e cobre contratos atuais', () => {
  ...
  assert.ok(tools.tools.some((item) => item.name === 'read_file' && item.category === 'read'));
  assert.ok(tools.tools.some((item) => item.name === 'run_command' && item.category === 'exec'));
  assert.ok(persistence.pathFactories.some((item) => item.name === 'configPath'));
  assert.ok(config.summary.topLevelFields >= 50);
  assert.equal(config.duplicates.length, 0);
  assert.ok(config.fields.some((item) => item.name === 'contextWindow'));
